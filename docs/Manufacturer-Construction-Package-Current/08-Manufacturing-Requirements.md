# Manufacturing Requirements – Current Manufacturer Package

## 1. Scope
These requirements apply to prototype and repeat builds of the Turbo V-Rod FT550 / ECUMASTER PMU16 harness set.

## 2. Manufacturer responsibility
The manufacturer is responsible for workmanship, correct application of specified connector systems, crimp quality, sealing, loom construction, electrical testing and accurate as-built documentation. Do not silently correct, reinterpret or substitute an engineering circuit. Raise an RFI for discrepancies.

## 3. Prototype measurement strategy
Several branch lengths remain vehicle-measurement gated. Preferred sequence:
...
## 4. Wire
Preferred low-current construction is M22759/32 ETFE or a documented equivalent motorsport thin-wall wire. High-current conductors must be sized from measured current/inrush, route length, voltage-drop target, ambient/loom derating and terminal capability.

**Fuel-pump controlling rule:** minimum 4.0 mm² feed and 4.0 mm² dedicated return per pump.

## 5. CAN
Use proper twisted pair. Preserve CAN H/CAN L polarity. Keep stubs short. Do not add hidden termination resistors. X51 service access must not alter normal bus termination.
...
## 7. Splices
Splices must be mechanically supported, sealed and staggered where appropriate. No household solder/twist/heatshrink construction. Any soldered joint required by a specific device must be explicitly approved.

## 8. Connectors and terminals
Use exact released connector families and terminal wire ranges. Do not crimp an undersize wire into an oversize terminal by folding the conductor. Use cavity plugs in unused sealed cavities where required. Where an OEM mating connector is unavailable, preserve the OEM pigtail and create the specified service break rather than cutting directly at the component.

## 9. Two-Step relay interface
Current production architecture bypasses the custom X70 PCB. Build a replaceable sealed relay sub-harness.
...
## 12. Testing
100% electrical test is required. At minimum verify intended continuity, no cross-circuit shorts, no unintended power-to-ground shorts, connector cavity identity, CAN polarity, X70 relay truth table and power/ground polarity.
